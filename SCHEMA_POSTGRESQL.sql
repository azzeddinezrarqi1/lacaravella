-- =====================================================
-- SCHÉMA POSTGRESQL - LA CARAVELLA
...
CREATE TABLE notifications (
    id SERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES auth_user(id) ON DELETE CASCADE,
    notification_type VARCHAR(20) NOT NULL CHECK (notification_type IN ('order_status', 'promotion', 'loyalty', 'newsletter', 'system')),
    title VARCHAR(200) NOT NULL,
...
CREATE TABLE user_preferences (
    ...
    preferred_shipping_method VARCHAR(20) DEFAULT 'standard' CHECK (preferred_shipping_method IN ('standard', 'express', 'pickup')),
    save_payment_info BOOLEAN DEFAULT FALSE,
    default_ice_cream_size VARCHAR(20) DEFAULT 'medium' CHECK (default_ice_cream_size IN ('small', 'medium', 'large')),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
BEGIN
    IF NEW.order_number IS NULL OR NEW.order_number = '' THEN
        NEW.order_number = 'CAR' || UPPER(SUBSTRING(gen_random_uuid()::text, 1, 8));
    END IF;
    RETURN NEW;
END;
$$ language 'plpgsql';
...
-- Trigger pour mettre à jour le niveau de fidélité
CREATE TRIGGER update_loyalty_tier_trigger BEFORE UPDATE ON user_profiles FOR EACH ROW EXECUTE FUNCTION update_loyalty_tier();

-- =====================================================
-- VUES UTILITAIRES
-- =====================================================

-- Vue des produits avec informations de catégorie
